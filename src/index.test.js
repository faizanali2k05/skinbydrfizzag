...
import { parseSSE, parsePartialJSON, streamJSON, streamSSEJson } from "./index.js";
...
test("parsePartialJSON: truncations at every prefix are safe and monotone", () => {
    ...
    for (let i = 1; i <= full.length; i++) {
        ...
        if (i === full.length)
            assert.deepEqual(v, JSON.parse(full));
    }
});
...
test("streamJSON yields growing snapshots then a done parse", async () => {
    ...
    const snaps = await collect(streamJSON(frags()));
    assert.equal(snaps.length, 3);
    assert.deepEqual(snaps[0].value, { story: "Once" });
    assert.deepEqual(snaps[1].value, { story: " upon".length ? "Once upon" : "" });
...
test("streamSSEJson: OpenAI-style deltas end-to-end", async () => {
    ...
    const sse = deltas.map((d) => `data: ${JSON.stringify({ choices: [{ delta: { content: d } }] })}\n\n`).join("")
        + "data: [DONE]\n\n";
    const snaps = await collect(streamSSEJson(chunks(sse), (e) => {
