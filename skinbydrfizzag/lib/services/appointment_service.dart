...
class AppointmentService {
  ...
  Stream<List<AppointmentModel>> getUserAppointmentsStreamByUserId(String userId) {
    return _supabase
        .from('appointments')
        .stream(primaryKey: ['id'])
        .eq('user_id', userId)
  ...
  Future<String?> rescheduleAppointment(
    ...
    try {
      ...
      final appointmentRes = await _supabase
          .from('appointments')
          .select('user_id, procedure_name')
          .eq('id', appointmentId)
          .maybeSingle();

      if (appointmentRes != null) {
        final procedureName =
            (appointmentRes['procedure_name'] as String? ?? 'Procedure');
        ...
        for (final row in adminRows) {
          ...
          await NotificationService.createNotification(
            ...
            message:
                'A user rescheduled $procedureName from $oldScheduledAt to $scheduledAt.',
            type: 'appointment',
            appointmentId: appointmentId,
          );
        }
