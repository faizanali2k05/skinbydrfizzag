...
class AuthService with ChangeNotifier {
  ...
  /// Creates a new user account. When called by an admin, this uses the
  /// Supabase Admin REST API so the admin's session is preserved.
  Future<String?> signUp({
    ...
    try {
      // Clean phone number: keep only digits so it seamlessly matches the WhatsApp API format (e.g. 92123456789 instead of +92 123 456789)
      final cleanPhone = phone.replaceAll(RegExp(r'\D'), '');

      // Normal users use standard signUp
      ...
      // Profile row creation is handled securely by the Supabase database trigger `on_auth_user_created`.
      // No manual upsert needed here, preventing any Row-Level Security (RLS) exceptions.
  ...
  Future<String?> updateUserPassword(String userId, String newPassword) async {
    try {
      ...
      debugPrint('AuthService: Attempting to update password for user: $userId');
      ...
      final response = await http.post(
        Uri.parse('https://skinbydrfizzag.onrender.com/admin/update-password'),
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success') {
          debugPrint('AuthService: Password updated successfully for user: $userId');
          return null;
        } else {
